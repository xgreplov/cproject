    ...
    public class ProductQueryTests
    {
        ...
        public async Task ExecuteAsync_ComplexWherePredicate_ReturnsCorrectQueryResult()
        {
            ...
            var predicate = new CompositePredicate(new List<IPredicate>
            {
                ...
                {
                    new SimplePredicate(nameof(Product.DiscountPercentage), ValueComparingOperator.GreaterThanOrEqual, 0),
                    new SimplePredicate(nameof(Product.DiscountPercentage), ValueComparingOperator.LessThan, 5)
                }),
                new CompositePredicate(new List<IPredicate>
                {
                    new SimplePredicate(nameof(Product.Price), ValueComparingOperator.GreaterThanOrEqual, 12_000m),
                    new SimplePredicate(nameof(Product.Price), ValueComparingOperator.LessThanOrEqual, 16_000m)
                })
            });
            using (unitOfWorkProvider.Create())
            {
        ...
        public async Task ExecuteAsync_RatingAllProductsByName_ReturnsCorrectlyOrderedQueryResult()
        {
            ...
            using (unitOfWorkProvider.Create())
            {
                actualQueryResult = await productQuery.SortBy(nameof(Product.Price), false).ExecuteAsync();
            }
        ...
        public async Task ExecuteAsync_ComplexQuery_ReturnsCorrectResult()
        {
            ...
            var predicate = new CompositePredicate(
                new List<IPredicate>{
                    ...
                    new CompositePredicate(new List<IPredicate>
                    {
                        ...
                        new CompositePredicate(new List<IPredicate>
                        {
                            new SimplePredicate(nameof(Product.Price), ValueComparingOperator.GreaterThanOrEqual, 15_000m),
                            new SimplePredicate(nameof(Product.Price), ValueComparingOperator.LessThanOrEqual, 16_000m)
                        })
                    }),
                });
            ...
            using (unitOfWorkProvider.Create())
            {
                actualQueryResult = await productQuery.Where(predicate).SortBy(nameof(Product.Price), false).Page(requestedPage, pageSize).ExecuteAsync();
            }
